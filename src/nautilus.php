...
class nautilus
{
    ...
    {   
        if(!file_exists($imageName)){
            throw new ImageException("Image " . $imageName . " does not exist");
        }
    ...
    public function uploadDir($directoryName, $filePermissions = 0666)
    {
        if (!file_exists($directoryName) && !is_dir($directoryName)) {
            ...
            if (!$createFolder) {
                throw new ImageException("Folder " . $directoryName . " could not be created");
            }
        }
    ...
    public function watermark($watermark, $watermarkPosition = null)
    {
        if (!file_exists($watermark)) {
            throw new ImageException(" Please provide valid image to use as watermark ");
        }
    ...
    protected function applyWatermark($imageName)
    {
        ...
        switch ($imageType) {
            ...
            default:
                throw new ImageException("A watermark can only be applied to: jpeg, jpg, gif, png images ");
                break;
        }
    }
    ...
    protected function applyShrink($fileName, $imageName)
    {
        ...
        switch ($mimeType) {
            ...
            case "gif":
                ...
                break;
            default:
                throw new ImageException(" Only jpg, jpeg, png and gif files can be resized ");
                break;
        }
    }
    ...
    protected function applyCrop($imageName, $tmp_name)
    {
        ...
        switch ($mimeType) {
            ...
            default:
                throw new ImageException(" Only gif, jpg, jpeg and png files can be cropped ");
                break;
        }
        ...
        if (!$temp) {
            throw new ImageException("Failed to crop image. Please pass the right parameters");
        } else {
    ...
    public function change($action, $imageName){
        ...
        if(empty($action) || !file_exists($imageName)){
            throw new ImageException(__FUNCTION__." needs two arguments. the Task and Image name");
        }
        ...
        throw new ImageException("Unknown directive given to function ". __FUNCTION__);
    ...
    public function deleteFile($fileToDelete){
        if (file_exists($fileToDelete) && !unlink($fileToDelete)) {
            throw new ImageException("File may have been deleted or does not exist");
        }
        return true;
    }
    ...
    public function upload($fileToUpload, $isNameProvided = null)
    {
       ...
       if(!function_exists('exif_imagetype')){
        throw new ImageException("Function 'exif_imagetype' Not found.");
       }

         // Check if any errors are thrown by the FILES[] array
        if ($fileToUpload["error"]) {
            throw new ImageException($this->commonUploadErrors($fileToUpload["error"]));
        }
        ...
        if (!in_array($this->getMimeType, $this->imageType)) {
            throw new ImageException(" This is not allowed file type!
        ...
        if ($fileToUpload["size"] < $this->imageSize["min"] ||
            ...
            throw new ImageException("File sizes must be between " .
        ...
        if($pixel["width"] < 4 || $pixel["height"] < 4){
            throw new ImageException("This file is either too small or corrupted to be an image");
        }

        if($pixel["height"] > $this->imageDimension["height"] || 
            $pixel["width"] > $this->imageDimension["width"])
        {
            throw new ImageException("Image pixels/size must be below ". 
        ...
        if ($moveUploadedFile) {
            ...
            throw new ImageException(" File could not be uploaded. Unknown error occurred. ");
        }
    }
